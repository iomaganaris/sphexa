 ...
 */
template<class KeyType, class T>
HOST_DEVICE_FUN inline KeyType sfc3D(T x, T y, T z, const Box<T>& box, unsigned bx, unsigned by, unsigned bz)
{
...
{
    return hilbertIBox<typename KeyType::ValueType>(keyStart, level, bx, by, bz);
}
...
{
    return sfcIBox(keyStart, keyEnd, maxTreeLevel<KeyType>{}, maxTreeLevel<KeyType>{}, maxTreeLevel<KeyType>{});
}
...
#pragma omp parallel for schedule(static)
    for (std::size_t i = 0; i < n; ++i)
    {
        ...
        {
            particleKeys[i] = sfc3D<KeyType>(x[i], y[i], z[i], box, mixDBits.bx, mixDBits.by, mixDBits.bz);
        }
    }
}
